...
class PaymentMenuView(QWidget):
    """결제 메뉴 화면 View 클래스"""
    # 할인 선택 시그널
    discount_requested = Signal(str)  # "student", "academy"
    # 결제 수단 선택 시그널
    pay_type_requested = Signal(str)  # "cash", "bank", "point"
    # 취소/이전 시그널
    ...
    def _connect_signals(self):
        ...
        self.ui.btn_student_discount.clicked.connect(
            lambda: self.discount_requested.emit("student")
        )
        self.ui.le_academy_discount.clicked.connect(
            lambda: self.discount_requested.emit("academy")
        )

        # 결제 수단 버튼
        self.ui.btn_cash_payment.clicked.connect(
            lambda: self.pay_type_requested.emit("cash")
        )
        self.ui.btn_bank_transfer_payment.clicked.connect(
            lambda: self.pay_type_requested.emit("bank")
        )
        self.ui.btn_academy_point_payment.clicked.connect(
            lambda: self.pay_type_requested.emit("point")
        )
