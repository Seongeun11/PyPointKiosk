...
class OrderMenuView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.CAT_COUNT = 6  # 2행카테고리 버튼 수 (동적)
        # 2줄 카테고리용 설정 (2행 x 4열 = 한 화면에 최대 8개)
        self.CAT_ROWS = 2
        self.CAT_COLS = 4
        self.VISIBLE_CAT_COUNT = self.CAT_ROWS * self.CAT_COLS  # 8개

        # [수정 1] 메인 Vertical Layout 반응형 레이아웃 Stretch 비율 설정
        # 카테고리가 2줄이 되었으므로 카테고리 영역 비율을 기존 1에서 2로 살짝 늘려 안정적인 공간 확보
        ...
        # 하단 결제 버튼의 Designer 고정 최소 높이 제한 해제 (반응형 확장을 위해)
    ...
    def _setup_scroll_areas(self):
        ...
        # ------------------------------------------------------------------
        # 1. 카테고리 영역 (좌/우 버튼 + 스크롤)
        # ------------------------------------------------------------------
        # 카테고리 전체를 감쌀 메인 레이아웃 (기존 self.ui.horizontalLayout 내에 배치)
        # 1. 카테고리 영역 (반응형 높이)
        ...
        # [변경] 기존 QHBoxLayout -> QGridLayout으로 변경 (2줄 배치용)
    ...
    def _update_ui_scaling(self):
        """창 크기에 비례하여 아이콘 및 UI 요소별 폰트 크기 자동 계산"""
        w = self.width()
        h = self.height()
        ...
        cat_font_size = max(10, min(int(13 * base_scale), 20))  # 2줄 배치를 고려해 폰트 약간 조정
    ...
    def render_categories(self, categories: list, current_idx: int):
        ...
        # 4개 이하일 때는 1줄, 4개 초과일 때는 2줄 구조 적용
        if cat_count <= self.CAT_COUNT:
            current_rows = 1
            btn_height = 45
            # 메인 레이아웃 Stretch 비율 (카테고리 영역: 1)
            self.ui.verticalLayout.setStretch(1, 1)
        else:
            current_rows = self.CAT_ROWS  # 2줄
            btn_height = 38
            # 메인 레이아웃 Stretch 비율 (카테고리 영역: 2)
            self.ui.verticalLayout.setStretch(1, 2)

        # 한 화면 기준(8개 = 2행 x 4열) 초과시에만 좌우 네비게이션 버튼 노출
    ...
    def render_products(self, products: list):
        ...
        # 뷰포트 높이에 맞춰 2행(8개) 규격에 알맞게 버튼 높이 조정
        ...
        for i in range(self.menu_grid_layout.count()):
            ...
            if layout_item is not None:
                ...
                if isinstance(widget, QToolButton):
                    ...
                    if btn_w <= 20 or btn_h <= 20:
                        continue

                    icon_dim = max(60, min(int(min(btn_w, btn_h) * 0.5), 200))
                    widget.setIconSize(QSize(icon_dim, icon_dim))

    def showEvent(self, event: QShowEvent):
        super().showEvent(event)
        # 화면이 처음 렌더링될 때 레이아웃 배치 완료 후 비동기 호출
        QTimer.singleShot(0, self._update_ui_scaling)

    def resizeEvent(self, event: QResizeEvent):
        super().resizeEvent(event)
        # 윈도우 크기가 변경되면 Qt가 자식 위젯 크기 배치를 마친 직후(SingleShot 0ms) UI 비율 반영
        QTimer.singleShot(0, self._update_ui_scaling)